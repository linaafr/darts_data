...
# Daten zusammengafasst (immer 3 Blöcke zusammengefasst)

3 Blöcke á 8 Würfe entspricht 24 Würfe 
...
means_wide <- darts_binned %>%
  select(vp, blockgruppe, mean_dist) %>%
  pivot_wider(names_from = blockgruppe, values_from = mean_dist, names_prefix = "mean_")

# Mediane transponieren
medians_wide <- darts_binned %>%
  select(vp, blockgruppe, median_dist) %>%
  pivot_wider(names_from = blockgruppe, values_from = median_dist, names_prefix = "median_")

# Zusammenführen
darts_transposed <- left_join(means_wide, medians_wide, by = "vp")

```

```{r}
# Mittelwerte transponieren
mean_transposed_3 <- darts_binned %>%
  select(vp, blockgruppe, mean_dist) %>%
  pivot_wider(
    names_from = blockgruppe,
    values_from = mean_dist
  )

# Mediane transponieren
median_transposed_3 <- darts_binned %>%
  select(vp, blockgruppe, median_dist) %>%
  pivot_wider(
    names_from = blockgruppe,
